...
"""
Dashboard Streamlit para Modelo FCSA MILP - Caio dos Santos
Otimização de estações de recarga rápida com PV
"""
...
def exibir_detalhes_solucao(modelo):
    ...
    # Abas para organizar informações (SEM ANÁLISE)
    tab1, tab2, tab3 = st.tabs(["💰 Custos", "⚡ Energia", "☀️ Sistemas PV"])
...
def main():
    ...
    # Título
    st.markdown('<h1 class="main-title">Otimização de eletropostos - modelo de Caio dos Santos</h1>', 
                unsafe_allow_html=True)
    ...
    if st.session_state.modelo_resolvido and st.session_state.modelo_atual:
        ...
        # Mapa em tela completa (SEM COLUNA DE RESUMO)        
        mapa = criar_mapa_resultados(modelo)
